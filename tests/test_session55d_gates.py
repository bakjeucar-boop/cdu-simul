"""세션 5.5-D — 다중 CDU 누출 주입 지점 정정 (미해결 #34).
...
**기준 ㉠ — 누출점이 계 전체에 하나다.** 다중 CDU 전이 케이스에서 누출 K값이
적용된 랙이 **CDU 를 통틀어 정확히 1개**여야 한다. 5-1 「누출 주입 지점」이
"랙 1개에 주입한다 · 전 랙 동시 누출은 5장에 근거가 없어 돌리지 않는다" 이므로,
