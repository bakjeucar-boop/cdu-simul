...
배분 +0.01905 L/s 였다. **그 두 수는 세션 5 시점의 관측이고 지금 모델의 값이
아니다** — 그 뒤 세션 7.61(펌프 일)과 7.72(UA 고정)가 온도를 옮겼고, 이 판은
그 자리를 다시 재지 않았다. **아래 검사가 보는 것은 부호(방향)뿐이다.**
**세션 5.5-B 의 양쪽 누출에서는 배분이 거의 움직이지
...
def test_criterion_4_b_return_temperature_falls_monotonically() -> None:
    ...
    였다(**세션 5 시점의 수이고 지금 모델의 값이 아니다** — 파일 머리말 참조).
    이 검사가 보는 것은 크기가 아니라 부호다. 부호가 뒤집히면 연동 경로가 잘못
    물린 것이다.
    """
