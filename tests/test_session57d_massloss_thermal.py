"""세션 5.7-D — 질량손실 열모델의 자기정합성 확인.

**게이트가 아니다.** 6장 기준을 하나도 판정하지 않는다. 이 파일이 고정하는 것은
`leak_massloss_thermal` 이라는 **별도 관측 코드**가 두 가지를 지키는가 하나다:

1. **격리** — 누출 0 에서 기존 밀폐루프 해를 그대로 재현하는가(정상 경로 무오염)
2. **balance** — 누출 엔탈피 항을 **넣어야** 닫히는가(빼면 닫히지 않아야 한다)

전수 스윕은 `python -m cdu_simul.leak_massloss_thermal` 이 돌린다 — 여기서는
...
from cdu_simul.leak_massloss import leak_topologies
from cdu_simul.leak_massloss_thermal import (
    leak_sizes_Lps,
...
@pytest.mark.parametrize("topology", leak_topologies(), ids=lambda t: t.label)
def test_leak_zero_reproduces_closed_loop(case, topology) -> None:
    """누출 0 이면 밀폐루프 해와 **부동소수점까지** 같아야 한다.

    다르면 이 모듈이 정상 경로를 건드린 것이다. 배치(g·펌프 유량 위치)는 누출이
...
@pytest.mark.parametrize("case", _CASES, ids=lambda c: c.label)
@pytest.mark.parametrize("topology", leak_topologies(), ids=lambda t: t.label)
def test_balance_closes_only_with_leak_enthalpy(case, topology) -> None:
    """누출 엔탈피 항을 넣어야 balance 가 닫힌다 — 판정 기준 D.
    ...
    assert abs(result.balance_residual_with_leak_percent) < 0.1
    assert abs(result.balance_residual_without_leak_percent) > 0.1
