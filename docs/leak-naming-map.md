...
즉 이 대응표는 **(가)가 열릴 때까지의 다리**다. (가)를 열 때 이 표가 그대로
변경 목록이 된다.
...
3. **「막힘」 이름은 건드리지 않는다** — 데이터셋 열·산출물에 닿아 있다.

## 표 1 — 세션 7.32 가 바꾼 이름 (old → new)
...
|---|---|---|---|
| `leak_level_percent` · `leak_rack_index` · `leak_cdu_index` · `leak_model` | 데이터셋 열 (52열본·48열본) | **「막힘」** (`leak_model` 만 상위) | **이미 세 벌로 나간 공개 면이다.** 바꾸면 48열본과 열 이름이 갈린다 |
...
| `LEAK_MODEL_K_APPROX` · `leak_model` | `dataset.py` | **상위(누출)** | 어느 기구로 모사했는지 밝히는 태그라 상위가 맞다 — 규약 위반이 아니다 |
...
- 「샘」에서는 **애초에 랙에 국소화되지 않기** 때문이다(랙 간 비대칭 2.220e-16 L/s ·
  세션 5.6). 「랙별 유량 하락으로 누출 랙을 특정」하는 판독은 「샘」에 대응물이 없다.
...
2. **「누출랙」 계열 낱말**(정리 126·127·128·129·134 · `project-overview.md` 268 ·
   ...
   낱말은 손대지 않았다.
3. **`massloss*.py` 두 모듈의 표시 문언에 남은 「누출」** — 「누출 크기」·「누출 0
   대비」 등. 「샘」 맥락이라 규약대로면 「샘」이지만, 이 판의 범위는 규칙 8 인용
   블록과 끊긴 인용뿐이었다.
