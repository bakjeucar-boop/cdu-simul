...
한 줄(세션 로그의 마지막 **판 제목** — 수준이 아니라 꼴로 가린다, 아래 파싱
규칙 참조) · ⓑ 열린 미해결 **번호와 제목만** ·
...
def _first_segment(text: str) -> str:
    """첫 ` · ` 앞까지 자른다 — **`(...)`·`「...」` 안의 ` · ` 는 경계로 보지 않는다**.

    세션 7.37. 대상은 반각 괄호 `()` 하나뿐이었다 — 「현재 상태」 표에서 ` · ` 를
    품는 괄호를 세어 보니 `(...)` 20건뿐이고 「」·[]·{} 안에는 없었다.
    세션 7.111(#60): 그 셈은 그날의 것이라 「」 안에 ` · ` 를 쓴 행이 들어오면
    재발한다 — `PROCEED.md` 전체에는 그런 「」 가 53건 있다. 그래서 `「」` 를
    깊이 계산에 더했다. `『』` 는 더하지 않았다 — 이 문서의 `『』` 3건이 전부
    「」 안에 있어 깊이가 이미 0 이 아니다.
    중첩은 깊이로 센다(실측 최대 2 · 두 종류를 한 깊이로 함께 센다).
    닫히지 않은 `(`·`「` 를 만나면 그 뒤에서
