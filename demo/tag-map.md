...
## 4. 닫힌 항목 — 세션 7.72 에 닫혔다

**52열본의 `heat_capacity_ratio` 열에는 Cr 이 아니라 ε(열교환기 유효도)가 들어
있었다(미해결 #41).** 세션 7.72 가 열 이름을 **`hx_effectiveness`** 로 바꿔
내용에 맞췄다 — **`heat_capacity_ratio` 라는 열은 지금 데이터셋에 없다.**
...
Cr 은 이 시연의 태그가 아니므로 시연 데이터에는 영향이 없었다. **화면이
데이터셋에서 Cr 을 읽으려 하면 안 된다 — Cr 열은 없다.**
(이 절의 기록을 닫힘으로 고친 것은 세션 7.110 — 미해결 #79.)
