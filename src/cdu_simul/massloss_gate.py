...
**energy balance 는 이 판정기가 재지 않는다 — 그러나 미판정은 아니다** [세션 7.54].
세션 7.53 이 계통 밖으로 나간 몫(`MassLossThermal.massloss_enthalpy_kW`)을 넣어
「샘」 13,824행의 잔차를 채웠고, **6장 ① 기준·임계를 그대로 적용해 전수 통과**했다
(|최대| 0.005087 % · 임계 0.1% 초과 0건). **지배방정식·solver 는 고치지 않았다** —
「그 항을 넣는 것은 물리 모델 변경이다」라던 세션 7.45 의 단서는 반증됐다.
이 파일은 그대로 기준 A·B·C 만 판정한다.
...
class Signal:
    ...
    `pump_sees_supply_flow` 가 참이면 하강(−) 거짓이면 상승(+)이다(세션 5.6 ·
    「샘」 주입점이 랙 출구일 때).
    """
    ...
    return [
        ...
        *(
            [
                ...
                "  정해지지 않았다. 리포트의 통과율도 같은 모집단으로 내고,",
                "  합격선(이 모집단 전수에서 통과율 100 % · 사람이 2026-09-08 에",
                "  정했다 · CLAUDE.md 게이트 표 세션 4 줄)도 이 모집단에서 판정한다.",
