...
#:
#: 이 데이터셋의 이상 상태는 **「막힘」(배관 K값 증가 · 5장 「누출 시나리오
#: (「막힘」)」)** 과 **「샘」(질량손실)** 둘이다(절대 규칙 8). 「샘」 행은 세션 7.39
#: 부터 실린다 — `massloss.py`·`massloss_thermal.py` 가 풀고 이 모듈이 둘을 import
#: 한다. 두 기구는 「샘」 주입점이 랙 출구일 때(이 데이터셋의 자리) 총유량·주입 랙
#: 유량의 **부호가 반대**이므로(세션 5.6 · 절대 규칙 8 · 미해결 #36 은 세션 7.68 에
#: 닫혔다) 이 열이 없으면 나중에 어느 쪽 데이터인지 알 수 없다. **열 이름은 세션 7.55 가
