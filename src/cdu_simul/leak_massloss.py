"""누출 **질량손실** 수력 모사 — 부호 확인 전용 (세션 5.6).

**절대 규칙 8 예외다 — 사람이 승인했다**(PROCEED.md 「사람이 정한 것 — 세션 5.5-D
마무리」⑶). 규칙 8 은 누출을 K값 변화로만 근사하게 하고, 이 파일은 그것을 어긴다.
허용 범위는 다음으로 한정된다:

- 규칙 8 **문구를 바꾸지 않는다.** 데이터셋과 5-1 「누출 시나리오」 정의는 K값
  근사 그대로다.
- **이 모듈은 데이터셋 생성 경로에 들어가지 않는다** — 저장소의 어느 모듈도 이
  파일을 import 하지 않는다. 기존 모듈은 **읽기만** 한다.
- 목적은 하나다: 데이터셋의 누출 신호(K 근사)가 **실제 누출(질량손실)과 같은
  부호를 가리키는가**.
...
랙 i 마다 한 식::
    ...
    Q_sup = ΣQ_i (헤더 공급유량)   ·   Q_ret = Q_sup - Q_leak (환수유량)

**누출 크기 Q_leak 은 주어진 값으로 강제한다.** 누출 오리피스를 모델링하면
...
class LeakTopology:
    """질량손실 누출의 계통 배치 — 5장·5-1 이 정하지 않는 구조 자유도 둘."""
...
def leak_topologies() -> tuple[LeakTopology, ...]:
...
class MassLossResult:
    ...
    def pump_flow_Lps(self) -> float:
        ...
        return self.supply_flow_Lps - self.leak_flow_Lps
...
def solve_massloss(
    ...
    `leak_flow_Lps = 0` 이면 식이 K 근사 정상 케이스와 **항등적으로 같다**
    ...
    if not converged:
        raise RuntimeError(
            ...
            f"{topology.label} / Q_leak={leak_flow_Lps:g} L/s — "
            f"{str(message).strip()}"
        )
    ...
    Q_pump = (
        Q_supply if topology.pump_sees_supply_flow else Q_supply - leak_flow_Lps
    )
...
def leak_flow_bound_Lps(k_results: list[FlowDistributionResult]) -> float:
    """스윕 상한 [L/s] — K 근사 +50% 가 만든 **누출랙 통과유량 감소량**.

    [유도: 5장 누출 3수준 · 5-1 「누출 주입 지점」 — 새 숫자를 만들지 않는다]
    """
...
class SignalDeltas:
    ...
    leak_rack_flow_change_Lps: float
...
def k_approx_deltas(
    ...
    normal, leaked = k_results[0], k_results[level_index]
    other = _other_rack_index(rack_index)
    total_change = leaked.total_flow_Lps - normal.total_flow_Lps
    return SignalDeltas(
        supply_flow_change_Lps=total_change,
        pump_flow_change_Lps=total_change,
        pump_head_change_mAq=leaked.pump_head_mAq - normal.pump_head_mAq,
        leak_rack_flow_change_Lps=(
            leaked.rack_flows_Lps[rack_index] - normal.rack_flows_Lps[rack_index]
        ),
        other_rack_flow_change_Lps=(
            leaked.rack_flows_Lps[other] - normal.rack_flows_Lps[other]
        ),
    )


def massloss_deltas(
    normal: MassLossResult, leaked: MassLossResult, rack_index: int
) -> SignalDeltas:
    """질량손실의 정상 대비 변화. 기준은 **같은 배치의 Q_leak=0 해**다."""
    other = _other_rack_index(rack_index)
    return SignalDeltas(
        supply_flow_change_Lps=leaked.supply_flow_Lps - normal.supply_flow_Lps,
        pump_flow_change_Lps=leaked.pump_flow_Lps - normal.pump_flow_Lps,
        pump_head_change_mAq=leaked.pump_head_mAq - normal.pump_head_mAq,
        leak_rack_flow_change_Lps=(
            leaked.rack_flows_Lps[rack_index] - normal.rack_flows_Lps[rack_index]
        ),
...
_QUANTITIES: tuple[tuple[str, str], ...] = (
    ...
    ("⑶ 누출랙 통과유량", "leak_rack_flow_change_Lps"),
...
def run_comparison(rack_index: int = LEAK.injection_rack_index) -> ComparisonRun:
    ...
    for case in cases:
        ...
        for topology in topologies:
            ...
            # 기준 C 격리 확인 — Q_leak=0 해가 K 근사 정상 해와 같아야 한다.
            isolation_diff = max(
                isolation_diff,
                max(
                    abs(a - b)
            ...
            for size_index, fraction in enumerate(_SWEEP_FRACTIONS[1:], start=1):
                ...
                n_solves += 1
                max_residual = max(
                    max_residual, leaked.max_abs_equation_residual_mAq
                )
                # 5-1 이 헤더를 저항 0 의 공통 노드로 두므로 랙 출구 누출은
                # 랙에 국소화될 수 없다 — 그 예상을 값으로 확인한다.
                asymmetry = max(
                    asymmetry,
                    max(leaked.rack_flows_Lps) - min(leaked.rack_flows_Lps),
                )
...
def format_comparison_table(run: ComparisonRun) -> str:
    ...
    """
    lines = [
        "세션 5.6 · 누출 K값 근사 대 질량손실 — 부호 대조 (수력 한정)",
        "※ " + ASSUMPTION_TAG,
        "※ **절대 규칙 8 예외** — 질량손실 모사는 이 파일에만 있다. 데이터셋과",
        "   5-1 「누출 시나리오」 정의는 K값 근사 그대로다.",
    ...
    lines += [
        ...
        "  (계통 밖으로 나가는 유량이 없다).",
        "",
        "[표 2] 질량손실 — 정상(Q_leak=0) 대비 변화 (8조합 × 크기 4수준 범위)",
        "  누출 크기는 조합마다 0 부터 **K 근사 +50% 가 만든 누출랙 유량 감소량**"
        " 까지 스윕했다.",
        f"  상한 범위: {min(run.leak_bounds_Lps):.6f} ~ "
        f"{max(run.leak_bounds_Lps):.6f} L/s (새 숫자가 아니다 — K 근사의 산출값).",
        "",
    ]
    ...
    lines += [
        "-" * len(header3),
        "",
        f"격리 확인(기준 C): Q_leak=0 의 질량손실 해와 K 근사 정상 해의 랙 유량"
