...
def plant_case_from_templates(
    ...
    필요해진 이유 [세션 5.5-D · 미해결 #34]: 다중 CDU 전이에서 「막힘」을 **한
    CDU 의 랙 1개**에만 걸어야 한다. 5-1 「「막힘」 주입 지점」이 "랙 1개에 주입한다
    · 전 랙 동시 「막힘」은 5장에 근거가 없어 돌리지 않는다" 이므로, 두 CDU 의 랙 0
    에 동시에 거는 것은 그 규정 밖이다. 정상상태 경로(`dataset.steady_rows`)는 이미 CDU 0
