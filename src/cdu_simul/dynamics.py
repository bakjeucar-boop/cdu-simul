...
@dataclass(frozen=True)
class LeakStepCase:
    """「막힘」 주입 시나리오 1건. 케이스마다 새로 만들어 초기조건을 리셋한다.

    부하는 **바뀌지 않는다** — 정격 정상운전 중에 「막힘」만 계단으로 들어온다.
    부하 스텝(`LoadStepCase`)과 자극이 다르므로 별도 케이스로 둔다.

    **이 경로의 이상 상태는 「막힘」뿐이다** — 배관 K값 배율이다(절대 규칙 8 ·
    5장). 「샘」(질량손실)은 여기 없다. `k_multiplier` 가 1.0 이면 **아무 일도
    일어나지 않는 정상 케이스**이고, 같은 경로로 돈다(세션 4 C2).
    """
