...
규칙 8 이 정한 두 이상 상태 중 하나이고, 이 파일은 그것을 푼다.

**남아 있던 마지막 조건도 세션 7.39 에서 풀렸다** — 「이 모듈은 데이터셋 생성
경로에 들어가지 않는다」가 그 조건이었는데, 사람이 「샘」 행을 데이터셋에 얹기로
정해 `dataset.py` 가 이 파일(과 `massloss_thermal.py`)을 import 한다. 이 파일은
여전히 기존 모듈을 **읽기만** 한다.

이 파일이 처음 답하려던 물음은 그대로다: 데이터셋의 「막힘」 신호(K값 증가)가
**「샘」과 같은 부호를 가리키는가** — 「샘」 주입점이 랙 출구일 때는 아니다. 부호
관계는 기구 × 주입점의 성질이다(절대 규칙 8 · 미해결 #36 은 세션 7.68 에 닫혔다).
...
def format_comparison_table(run: ComparisonRun) -> str:
    ...
    lines = [
        ...
        "   「샘」 모사는 이 파일에 있다. **데이터셋에도 세션 7.39 부터 「샘」 행이",
        "   실린다**(정상상태 한정 · 전이는 미해결 #40).",
