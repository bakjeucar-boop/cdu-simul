...
#: [규약: 프로젝트정리 5-1 「물성 조회 기준 압력」 — **확정**(세션 5)]
#:
#: 5-1 은 이 값을 **확정**으로 적는다. 근거는 세션 5 가 코드로 확인한 것이다:
#: 계통 압력(펌프 양정 20~30 mAq = 196,133~294,200 Pa)은 대기압 대비
#: **193.6~290.4%** 로 두 자릿수 큰 값이지만, CoolProp INCOMP 백엔드에서
#: **ρ·cp 는 압력에 정확히 무관**하고(양 끝에서 0.0000% 변화) energy balance 가
#: 쓰는 **엔탈피 차만 −0.0138%** 이동한다(6장 기준 0.1% 의 1/7). 그래서 압력
#: 의존을 모델에 넣지 않고 이 상수를 조회용 기준으로만 쓴다.
#:
#: **한계(5-1)**: 압력이 실제로 물성에 걸리는 유체(2상·기체)로 바뀌면 이 확정은
#: 무효다.
