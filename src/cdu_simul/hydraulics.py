...
class HydraulicCase:
    ...
    조건이다. 「막힘」(절대 규칙 8: 배관저항 K값 증가)이 이 자리에 걸리지만
    **「막힘」 구현은 세션 4**이며, 세션 3-A 에서는 방향성 검사에만 쓴다.
    """
...
def apply_leak_to_rack(
    ...
    """랙 하나의 배관 K값에 배율을 걸어 **「막힘」** 케이스를 만든다 (순수 함수).

    [절대 규칙 8 · 5장 「누출 시나리오(「막힘」)」 · 5-1 「「막힘」 주입 지점」 ·
    세션 4]

    이 함수가 만드는 것은 **「막힘」 하나뿐이다** — 배관저항(K값) 증가이고 질량은
    보존된다. 「샘」(질량손실)은 여기 없다(별도 모듈 `massloss.py`). 3-A 가 만들어
    둔 `branch_K_multipliers` 진입점을 쓰며 새 메커니즘을 만들지 않는다. 「샘」 쪽
    관측 모듈이 이 함수를 **빌려 쓴다** — 「막힘」 해를 나란히 놓기 위해서다
    (세션 7.32 · 대응표 `docs/leak-naming-map.md`).

    **배율 1.0(정상)도 이 함수를 통과한다.** 정상과 「막힘」이 다른 경로를 타면 결과
    차이가 「막힘」 때문인지 경로 때문인지 갈라낼 수 없다(세션 4 C2). 배율 1.0 이면
    `rack_branch_K` 가 원래 값과 부동소수점까지 같다(1.0 곱은 항등이다).

    라벨에 배율을 남긴다 — 결과 표에서 어느 케이스가 「막힘」인지 보이게 하려고.
    """
