...
def _component_map() -> list[tuple[str, str, str]]:
    ...
    return [
        (
            ...
            "**수력동력 P_hyd 는 세션 7.61 부터 열로 계상돼 두 노드에 나뉘어 "
            "들어간다**(5-1 「펌프 일의 노드 배분」) — 효율·모터 발열은 모델에 없다",
        ),
        (
            "밸브",
        ...
        (
            ...
            "배관 K값 증가 근사(5장 정의). **다른 이상 기구인 「샘」(질량손실)은 "
            "별도 경로로 모델에 있다** — `massloss.py`·`massloss_thermal.py` · "
            "데이터셋에는 세션 7.39 부터 실린다(정상상태 한정 · 전이는 미해결 #40)",
        ),
    ]
...
def _gate_results() -> list[tuple[str, str, str]]:
    ...
    return [
        (
            ...
            "환수유량과 계통 밖으로 나간 유량으로 갈리는 몫만 「샘」 엔탈피 항으로 "
            "더한다(`model.enthalpy_balance_residual_percent`). "
            "**여기서 잰 최대 |잔차| 는 「막힘」·정상 32조합의 것이다** — 전 행의 "
            "분포는 이 리포트가 재지 않는다. "
            "**[세션 7.64] 잔차식 오른쪽에 든 항**: 랙 발열 + **펌프 수력동력 중 "
            "랙 다리 몫**(세션 7.61 부터 · 5-1 「펌프 일의 노드 배분」). "
            "**계통 방열 · 펌프 효율 · 모터 발열은 잔차식에도 모델에도 없다** — "
            "방열은 세션 7.62 가 크기만 어림했고(이 임계의 0.6~78.3배 · 보온 "
            "사양에 따라 갈린다 · `scripts/measure_ambient_heat_loss.py`) 넣지 "
            "않았다. 넣는 항만 있고 빼는 항이 없다는 뜻이다",
        ),
        (
            "6장 ② T_return 방향성",
            "통과 (8랙 16조합)",
        ...
        (
            ...
            "표본 검사다(계획 9,472 중 stride 61 — 세션 7.39 에서 「샘」 7,680 이 "
...
OPEN_LIMITS: tuple[tuple[str, str], ...] = (
    ...
    (
        "어느 「샘」 주입점이 실제 누출에 가까운가 — 실측이 없어 이 파일럿이 "
        "답하지 않는다 (미해결 #36 은 세션 7.68 에 닫혔다)",
        "`total_flow_Lps` · `rack{i}_flow_Lps` · `rack{i}_outlet_C` · "
        "`pump_head_mAq`. 「막힘」·「샘」의 부호 관계는 기구의 성질이 아니라 "
        "기구 × 「샘」 주입점의 성질이다(절대 규칙 8). 이 데이터셋의 주입점은 "
        "랙 출구 하나이고 사람이 2026-09-07 에 그대로 두기로 정했다 — 그 자리에서 "
        "읽는 법은 위 ③ 절 「두 기구를 읽는 틀」에 있다. 주입점을 옮긴 경우의 "
        "방향은 이 데이터셋이 말하지 않는다. `anomaly_mechanism` 열이 **행마다** "
        "어느 기구인지 싣는다 — `K_approx`(「막힘」)와 `massloss`(「샘」)가 "
        "세션 7.39 부터 한 표에 있다",
    ),
    (
    ...
    (
        ...
        "`ntu` 열은 **정격점 값**(5장 축값)이고 운전 NTU 는 `ntu_operating` "
        "열에 있다(세션 7.86 신설). "
...
def format_report() -> str:
    ...
    lines += [
        ...
        "",
        "  ── 무엇 **안에서** 구분되는가 (세션 5.6 · 5.6-B · #36 은 7.68 에 닫혔다) ──",
        ...
        "    **「막힘」**이다. 이 판정을 「샘」 행으로 옮겨 읽지 않는다 — 까닭은",
        "    이 절 끝의 「두 기구를 읽는 틀」이다.",
        "  · **세션 7.39 부터 이 데이터셋에 「샘」 행이 함께 있다**",
        "    (`anomaly_mechanism=massloss`). 「샘」은 세션 7.78 부터 게이트를 진다 —",
        "    사람이 2026-09-08 에 기준 A·B·C(docs/session745-massloss-gate-criteria.md)",
        "    를 「이상 기구를 진 CDU」 전수에서 통과율 100 % 로 올렸고(CLAUDE.md 게이트",
        "    표 세션 4 줄), 시험 test_massloss_gate_passes_on_every_leak_cdu_pair 가",
        "    이 CSV 로 판정한다. 이웃 CDU 는 모집단에 없다. 통과의 뜻은 「모델 안에서",
        "    신호가 잡음 위에 있다」까지다 — 실측 감지 가능성이 아니다. 판정된 짝의",
        "    최소 여유(원단위 · 해당 없음 짝 제외)는 세션 7.119 가 이 판본 CSV 에서 잰",
        "    값으로 기준 B 3.654e-04 · 기준 C 온도 짝 1.213e-03 이다(이 리포트가 재는",
        "    수가 아니다). 행마다 `anomaly_mechanism` 을 보고 어느 기구인지 갈라 읽는다.",
        "  · 「샘」 주입점이 랙 출구일 때(이 데이터셋의 자리) 질량손실(계통 밖 유출)로",
        "    모사하면 **총유량과 주입랙 통과유량의 부호가 정반대**로 나온다 — K 근사는",
        "    둘 다 감소, 질량손실은 둘 다 증가다",
        "    (세션 5.6 · 8조합 × 크기 4수준 × 배치 6 전수 · fsolve 272회 ier=1).",
        "    **펌프 양정의 부호는 5장·5-1 이 정하지 않는 잔여저항 배치에 따라",
        "    갈린다**(#24 참조).",
        "  · 일부 배치(잔여저항이 전부 「샘」 주입점 상류 · 펌프가 공급유량을 봄)에서는",
        "    「샘」이 **수력 흔적을 전혀 남기지 않는다**(다섯 양 전부 정확히 0",
        "    · 세션 5.6 관측 ④ · 주입점이 랙 출구일 때).",
        ...
        "    **[세션 7.41 정정] 59열본에는 이 Δ 가 열로 들어와 있다** —",
        ...
        "  · → **읽는 법**: 「총유량이 줄면 이상」과 「랙별 유량 하락으로 막힘 랙을",
        "    특정」은 이 데이터셋의 「막힘」 행 안에서 성립하는 규칙이다. 「샘」 행으로",
        "    옮기지 않는다 — 아래 틀의 조건에서 「샘」은 총유량을 늘린다. 어느 주입점을",
        "    둘지는 이 파일럿이 답하지 않고, 사람이 2026-09-07 에 랙 출구로 정했다",
        "    (미해결 #36 은 세션 7.68 에 닫혔다).",
        "",
        "  ── 두 기구를 읽는 틀 (절대 규칙 8) ──",
        "  · 「막힘」(배관 K값 증가 · 질량 보존)과 「샘」(계통 밖 질량손실)은 서로의",
        "    대용이 아니라 각각 **독립된 이상 상태**다(절대 규칙 8). 「누출」은 둘을",
        "    묶는 상위 개념이고, 모델 안의 기구는 언제나 「막힘」·「샘」으로 적는다.",
        "  · 두 기구 신호의 부호 관계는 **기구의 성질이 아니라 기구 × 「샘」 주입점의",
        "    성질**이다. 이 모델과 데이터셋의 「샘」 주입점은 **랙 출구 하나**다",
        "    (5-1 「「샘」 주입 지점」). 미해결 #36 은 세션 7.68 에 닫혔고, 사람이",
        "    2026-09-07 에 주입점을 랙 출구 그대로 두기로 정했다.",
        "  · 랙 출구 주입 · 정상상태 · 이상 기구를 진 CDU(단일 CDU · 다중 CDU 모두)",
        "    에서 총유량과 주입랙 통과유량은 「막힘」이 줄이고 「샘」이 늘리며,",
        "    주입랙 출구온도는 「막힘」이 올리고 「샘」이 내린다. 구조 자유도 배치",
        "    `residual_return_share=0.0` · `pump_sees_supply_flow=True` 에서는 「샘」의",
        "    수력 신호를 판정하지 않는다(「해당 없음」 · 세션 7.47) — 온도는 그",
        "    배치에서도 응답한다. 주입점을 옮긴 경우 · 전이 · 다중 CDU 의 이웃 CDU",
        "    에 대해서는 이 틀이 방향을 말하지 않는다.",
        "  · 그래서 한 기구의 신호를 다른 기구의 방향으로 읽지 않는다.",
        "    `anomaly_mechanism` 을 읽지 않은 채 「유량 감소 = 이상」 같은 규칙을",
        "    세우지 않는다.",
        "",
        ...
        "",
        "  ── 열·전이까지 올려도 부호가 갈린다 (세션 5.7-D · 「샘」 주입점 = 랙 출구) ──",
        ...
        "  · 에너지 balance 는 **「샘」 엔탈피 항을 넣어야 닫힌다** — 빼면 최대",
    ...
    lines += [
        ...
        f"  52열(세션 5.7) · 59열(`{DATASET_VERSION}` · 세션 7.39 · 58열로",
        "  나갔다가 세션 7.86 이 `ntu_operating` 을 더했다)**.",
        "  `dataset_version` 열의 **부재 자체가 5.7 이전 판본이라는 표지**다.",
        "  · 세션 7.39 에서 **「샘」(질량손실) 행이 처음 들어왔다** — 이전 판본은",
        "    전부 「막힘」(K값 증가)뿐이었다. 이 판본의 「샘」 주입점(랙 출구)에서 두",
        "    기구는 부호가 반대이므로(③ 절 「두 기구를 읽는 틀」 · 미해결 #36 은 세션",
        "    7.68 에 닫혔다) **`anomaly_mechanism` 을 읽지 않고 두 판본을 나란히 읽지",
        "    않는다.**",
        ...
        "  · **어느 「샘」 주입점이 실제 누출에 가까운가** — 실측이 없다. 두 기구의",
        "    부호 관계가 주입점에 매이는데(절대 규칙 8) 어느 자리를 둘지는 이",
        "    파일럿이 답하지 않는다. 사람이 2026-09-07 에 랙 출구로 정했다(미해결",
        "    #36 은 세션 7.68 에 닫혔다)",
