...
OPEN_LIMITS: tuple[tuple[str, str], ...] = (
    ...
    (
        ...
        "전이에서 「막힘」의 효과를 보려면 같은 부하 스텝 안에서 `leak_level_percent` 를 "
        "가로질러 비교한다. 다중 전이도 「막힘」을 스텝으로 넣을지는 "
        "**사람이 정할 사항**이다",
    ),
    (
        "「막힘」이 걸리는 CDU 가 **부하가 움직이는 쪽**으로 고정돼 있다 (세션 5.5-D)",
        "`leak_cdu_index` 는 「막힘」이 있는 행에서 항상 0 이고, 비대칭 전이에서 "
...
def format_report() -> str:
    ...
    lines = [
        ...
        "  정상상태 행에는 「막힘」 축(`leak_level_percent`)이 **그대로 살아 있다.**",
        "",
