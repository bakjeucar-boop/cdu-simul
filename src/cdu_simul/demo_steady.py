...
안 된다** — 두 기구의 부호 관계는 기구 × 「샘」 주입점의 성질이고, 주입점이 랙
출구일 때(모델의 자리) 세션 5.6 수력 · 세션 5.7-D 열에서 부호가 반대로 나왔다
(절대 규칙 8 · 미해결 #36 은 세션 7.68 에 닫혔다).
...
CASE_CAVEATS: dict[str, str] = {
    ...
    "load_sampling": (
        ...
        "5-1 에 넣지 않았다. 데이터셋(59열본)은 여전히 양 끝 2수준이므로 둘을 "
        "섞어 읽지 않는다"
    ),
    ...
    "resistance_blockage": (
        ...
        "구분을 「샘」의 방향으로 읽으면 안 된다 — 두 기구의 부호 관계는 기구 × "
        "「샘」 주입점의 성질이고, 주입점이 랙 출구일 때(모델의 자리 · 2026-09-07 "
        "사람의 결정) 세션 5.6 수력 · 세션 5.7-D 열에서 부호가 정반대로 나왔다"
    ),
    "total_flow_caveat": (
        "총유량만 보면 +50% 도 −0.16~0.37% 라 놓칠 수 있다(세션 4 관측 ③). "
        "랙별 유량·랙별 출구온도와 함께 읽는다. "
        "**이 수치도 처방도 「막힘」 것이다** — 이 화면은 「막힘」만 싣는다"
        "(resistance_blockage 표기). 「샘」은 주입점이 랙 출구일 때(모델의 자리 · "
        "2026-09-07 사람의 결정) 총유량을 **반대 방향(증가)** 으로 움직이고 랙에 "
        "국소화되지 않아 랙별 열이 어느 랙인지 가리키지 못한다(절대 규칙 8) — "
        "이 문언을 「샘」 쪽으로 옮겨 읽지 않는다"
    ),
    "fixed_axes": (
    ...
    return {
        ...
        "meta": {
            ...
            "rack_length_note": (
                ...
                "데이터셋(59열본)과 기존 게이트 기록은 균등 그대로다"
            ),
