...
(랙 배관 K값 증가 · 절대 규칙 8 근사). 파일럿 종료 판정 5-b 가 **X** 이기
때문이다 — 데이터셋의 구분이 실제 누출의 방향과 같지 않다(세션 5.6 수력 ·
세션 5.7-D 열에서 부호가 정반대로 나왔다). `leak_model` 표기를 함께 싣는다.
...
#: 저항 증가를 거는 CDU. 5-1 「누출 주입 지점」이 랙 1개로 정하고 있고,
...
CASE_CAVEATS: dict[str, str] = {
    ...
    "resistance_proxy": (
        "resistance_increase_percent 는 **랙 배관 K값 증가**이며 누출의 대용이다"
        "(절대 규칙 8 근사). 같은 것이 아니다 — 파일럿 종료 판정 5-b 가 X 다: "
        "데이터셋의 구분이 실제 누출의 방향과 같지 않다(세션 5.6 수력 · "
        "세션 5.7-D 열에서 부호가 정반대로 나왔다)"
    ),
    "total_flow_caveat": (
