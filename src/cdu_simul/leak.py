"""「막힘」 시나리오 실행 (세션 4) — 정상 대비 신호 패턴.

범위 — 4장 「단계적 확장 전략」 3단계다.

    정상 · 「막힘」 +5/+20/+50% → 랙 유량 · 랙 출구온도 · 펌프 운전점의 변화

**이 모듈이 다루는 이상 상태는 「막힘」이다**(절대 규칙 8 · 5장 「누출 시나리오
(「막힘」)」) — 배관저항(K값) 증가이고 질량은 보존된다. **「샘」의 대용(proxy)이
아니라 독립된 이상 상태다**(세션 7.26 이 규칙 8 을 그렇게 고쳤다). 「샘」(질량
손실)은 이 경로에 없다 — 별도 관측 모듈 `massloss.py`·`massloss_thermal.py` 가
풀며 **「샘」 주입점이 랙 출구일 때 부호가 반대다**(절대 규칙 8 · 미해결 #36 은
세션 7.68 에 닫혔다). 여기 나오는 신호를 「샘」의 방향으로 읽지 않는다.

**정상(배율 1.0)이 같은 코드 경로로 돈다.** `LEAK.k_multiplier_levels` 가 정상을
첫 수준으로 담고 있고, 정상도 `apply_leak_to_rack` 를 통과한다 — 정상과 「막힘」이
다른 경로를 타면 결과 차이가 「막힘」 때문인지 경로 때문인지 갈라낼 수 없다.

**주입 지점은 랙 1개**다 [시나리오 정의: 5-1 「「막힘」 주입 지점」 · 세션 4].
8랙이 전부 동일하므로 랙 번호는 결과에 영향하지 않는다. 전 랙 동시 「막힘」은
5장에 근거가 없어 돌리지 않는다.
