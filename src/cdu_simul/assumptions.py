...
class ValveAssumptions:
    ...
    # ── Kv (개도 100% 기준) ──────────────────────────────────────────────────
    # [역산: 프로젝트정리 5-1 「밸브 Cv (개도 100% 기준)」 · 세션 3 확정]
    # 5-1 에 적힌 숫자를 그대로 전사한다 — 여기서 다시 역산하지 않는다.
    # 근거(5-1): 5장 역산 조건(랙당 1.94 L/s · 개도 80% 에서 ΔP 3~5 mAq)에
    # Kv = Q*sqrt(SG/ΔP) 를 적용해 80% 값을 구하고 선형 특성으로 100% 로
    # 환산했다. SG = 1.0124 (37℃ 벌크평균 ρ 기준). 5장 밖의 새 수치를 넣은 것이
    # 아니라 **5장 조건을 역산한 결과**다.
    # 5장 ΔP 가 범위(3~5 mAq)이므로 Kv 도 **양 끝 두 값**이다 — ΔP 가 클수록
    # 같은 유량을 흘리는 데 필요한 Kv 는 작아진다.
    # 병기된 Cv(18.72 / 14.50)는 Kv/0.865 로 같은 값의 다른 단위계 표기이며,
    # 계산은 Kv(m^3/h) 하나로만 한다(절대 규칙 9 — 같은 물리량을 두 단위로 섞지
    # 않는다).
    Kv_max_m3h_at_dP_low: float = 16.19  # 5장 ΔP 하한 3 mAq 에 대응 (Cv 18.72)
    Kv_max_m3h_at_dP_high: float = 12.54  # 5장 ΔP 상한 5 mAq 에 대응 (Cv 14.50)

    @property
    def Kv_max_bounds_m3h(self) -> tuple[float, float]:
        """5장 밸브 ΔP 범위 양 끝에 대응하는 Kv_max 두 값 (ΔP 하한, ΔP 상한)."""
        return (self.Kv_max_m3h_at_dP_low, self.Kv_max_m3h_at_dP_high)
...
class PipingAssumptions:
    ...
    # ── 배관 K값 (랙 분기) ───────────────────────────────────────────────────
    # [역산: 프로젝트정리 5-1 「배관 K값 (랙 분기)」 · 세션 3 확정]
    # 5-1 에 적힌 숫자를 그대로 전사한다 — 여기서 다시 역산하지 않는다.
    # 근거(5-1): 5장 "랙당 ΔP 2~3 mAq @ 1.94 L/s" 와 5-1 배관 내경만으로
    # ΔP = K*rho*v^2/2 를 역산했다. 새 가정치 없음. 무차원이며 **25A 내경 기준
    # 유속으로 정의**된다 — 다른 구경에 그대로 쓰면 안 된다.
    # 누출 시나리오의 K값 +5/+20/+50%(`LEAK`)가 이 값에 걸린다(절대 규칙 8).
    # **관측(5-1)**: 25A 에 1.94 L/s 는 유속 3.48 m/s 로 냉각수 통상 설계유속
    # (2~3 m/s)보다 높다 — 설계데이터 확보 시 랙 분기 구경이 커질 가능성이 있고,
    # 그러면 K 와 M 이 함께 이동한다.
    # 범위 양 끝은 5장 ΔP 범위 양 끝(2 mAq → 3.20 · 3 mAq → 4.80)에 대응한다.
    rack_branch_K: Range = Range(3.20, 4.80, "-")
