...
class LeakScenarioAssumptions:
    ...
    # ── 주입 지점 ────────────────────────────────────────────────────────────
    # [시나리오 정의: 프로젝트정리 5-1 「누출 주입 지점」 · 세션 4 확정]
    # **랙 1개**에 주입한다. 5장 누출 항목이 몇 개 랙인지 말하지 않고, 이 과제의
    # 목적이 미량 누출 조기감지(1장)이므로 국소 누출이 그 목적에 맞다. 8랙이 전부
    # 동일하므로 랙 번호는 결과에 영향하지 않는다 — 새 숫자를 만드는 것이 아니다.
    # **물리 가정이 아니라 시나리오 정의**다. 전 랙 동시 누출은 5장에 근거가 없다.
    ...
    def k_multiplier_levels(self) -> tuple[tuple[str, float], ...]:
        ...
        **같은 코드 경로**로 돌아야 결과 차이가 누출 때문인지 경로 때문인지 갈라낼
        ...
        절대 규칙 8: 누출은 배관저항(K값) 변화로만 근사한다. 질량 손실을 직접
        모델링하지 않는다.
        """
        return (("정상", 1.0),) + tuple(
