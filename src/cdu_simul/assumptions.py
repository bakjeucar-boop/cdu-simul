...
#: 열수지에 **든 항과 안 든 항** — 산출물 문언 한 벌 [세션 7.64].
#: `ASSUMPTION_TAG` 와 같은 자리에 두는 이유는 위와 같다: 표시 문구가 모듈마다
#: 갈리면 어느 표에는 빠지게 된다(collaboration.md ③).
#: 방열 크기는 **세션 7.62 어림값의 인용**이며 5-1 이 채택한 값이 아니다.
HEAT_TERMS_CAVEAT: str = (
    "열수지에 든 항은 랙 발열과 **펌프 수력동력**이다 — 후자는 세션 7.61 부터 "
    "열로 계상돼 두 노드에 나뉘어 들어간다(5-1 「펌프 일의 노드 배분」). "
    "**계통 방열 · 펌프 효율 · 모터 발열은 모델에 없다** — 방열은 세션 7.62 가 "
    "크기만 어림했고(6장 ① 임계의 0.6~78.3배 · 보온 사양에 따라 갈린다) "
    "넣지 않았다"
)

#: 세션 3-B 산출물에 붙이는 과대해석 금지 문구 (절대 규칙 11 · 6장).
#: `ASSUMPTION_TAG` 와 같은 자리에 두는 이유: 둘 다 **산출물 표시**이고, 표시
#: 문구가 모듈마다 갈리면 어느 표에는 빠지게 된다(collaboration.md ③).
SESSION_3B_CAVEAT: str = "\n".join(
    [
        "※ 이 판이 판정한 것 (과대해석 금지)",
        "· 8랙은 전부 동일 조건이고 유량이 균등하므로, 열적으로는 1랙 집중모델과",
        "  동치다. \"8랙 거동을 확인했다\"로 읽지 않는다 — 랙 간 비대칭은 누출 K값이",
        "  들어오는 세션 4에서 처음 생긴다",
        "· 유량분배 결과는 잔여저항 배정 방식이 지배한다(총양정의 60~83%, 5-1 한계).",
        "  수렴·비발산 통과를 \"유량분배 값이 타당하다\"로 읽지 않는다",
        "· 수렴시간은 여전히 판정하지 않는다 — M 에 HX·CDU 내부·콜드플레이트 보유량이",
        "  빠져 있다(미해결 #21)",
    ]
)

#: 세션 4 산출물에 붙이는 과대해석 금지 문구 (절대 규칙 11 · 6장).
#: `ASSUMPTION_TAG` · `SESSION_3B_CAVEAT` 와 같은 자리에 둔다 — 표시 문구가
#: 모듈마다 갈리면 어느 표에는 빠지게 된다(collaboration.md ③).
SESSION_4_CAVEAT: str = "\n".join(
    [
        "※ 이 판이 판정한 것 (과대해석 금지)",
        "· 이 판이 돌린 이상 기구는 「막힘」(배관 K값 증가 · 5장 정의) 하나이며,",
        "  K값 증가는 근사이지 「막힘」 자체의 물리가 아니다. **다른 이상 기구인",
        "  「샘」(질량손실)은 별도 경로로 모델에 있고**(세션 5.6·5.7-D) 데이터셋에도",
        "  세션 7.39 부터 실린다 — 이 판이 돌리지 않았을 뿐이다",
        "· " + HEAT_TERMS_CAVEAT,
        "· 신호 크기는 잔여저항 배정 방식에 지배된다(총양정의 60~83%, 5-1 한계)",
        "· 전이 시간 규모의 절대값은 해석하지 않는다 — #31(8랙 해석)은 세션 5.7 에서",
        "  닫혔으나 M 결손(#21) · 등가길이 과대(#25) · 배관 규격 계열(#23)이 열려 있다",
        "· 랙 1개 누출만 돌렸다. 전 랙 동시 누출은 5장에 근거가 없어 돌리지 않았다",
    ]
)
...
SESSION_5_CAVEAT: str = "\n".join(
    [
        ...
        "· 이 판이 돌린 이상 기구는 여전히 「막힘」(배관 K값 증가 · 5장 정의)",
        "  하나다. **「샘」(질량손실)은 별도 경로로 모델에 있고**(세션 5.6·5.7-D)",
        "  데이터셋에도 세션 7.39 부터 실린다. 「샘」 주입점이 랙 출구일 때 두 기구는",
        "  이상 기구를 진 CDU 의 총유량 부호가 반대이므로(절대 규칙 8 · 미해결 #36 은",
        "  세션 7.68 에 닫혔다) 이 판의 부호를 「샘」 쪽으로 옮겨 읽지 않는다",
        "· " + HEAT_TERMS_CAVEAT,
        "· 수렴시간은 판정하지 않는다(#21·#25·#23 — #31 은 세션 5.7 에서 닫혔다)",
    ]
)
...
class PumpAssumptions:
    ...
    # ── 펌프 수력동력의 노드 배분 [-] ─────────────────────────────────────
    # [규약: 프로젝트정리 5-1 「펌프 일의 노드 배분」 — 물리 가정이 아니라
    #  수치처리 규약이다. `[가정값]` 이 아니다]
    # 잔여저항(HX 1차측 + CDU 내부배관 + 헤더)이 만든 몫 중 **공급 노드**로
    # 가는 비율. 환수 노드 몫은 `1 - 이 값` 이다(둘의 합이 1 로 항등).
    # 랙 분기 ΔP·밸브 ΔP 가 만든 몫은 배분 대상이 아니다 — 환수 노드 100% 다.
    #
    # 값 0.5 는 5-1 「계통 보유수량 M의 노드 배분」과 **근거가 같고 값이 같으나
    # 같은 양이 아니다** — 이쪽은 펌프 일의 노드 배분이고 저쪽은 열용량의 공간
    # 배분이다. 그래서 `PIPING.holdup_*_node_fraction` 을 참조하지 않고 **독립된
    # 리터럴**로 둔다(세션 7.63): M 배분을 바꾸는 결정(#40)이 펌프 열 배분을
    # 말없이 따라 움직이지 않게 하기 위해서다.
    heat_residual_supply_node_fraction: float = 0.5
...
class PipingAssumptions:
    ...
    # 쓰면 안 된다. 「막힘」의 K값 +5/+20/+50%(`LEAK`)가 이 값에 걸린다
    # (절대 규칙 8 · 5장 「누출 시나리오(「막힘」)」).
...
class LeakScenarioAssumptions:
    """5장 구성요소 표 — 「누출 시나리오(「막힘」)」.

    절대 규칙 8: **「막힘」은 배관저항(K값) 증가**이며 질량이 보존된다. 이 클래스가
    담는 것은 「막힘」뿐이다 — 「샘」(질량손실)의 크기는 여기 없고 5-1 「「샘」
    (질량손실) 크기 수준」의 역산 규칙이 `massloss.py` 에서 낸다. 두 기구를
    같은 이름으로 읽지 않는다(대응표 `docs/leak-naming-map.md`).
    """
    ...
    # ── 주입 지점 ────────────────────────────────────────────────────────────
    # [시나리오 정의: 프로젝트정리 5-1 「「막힘」 주입 지점」 · 세션 4 확정]
    # **랙 1개**에 주입한다. 5장 누출 항목이 몇 개 랙인지 말하지 않고, 이 과제의
    # 목적이 미량 누출 조기감지(1장)이므로 국소 이상이 그 목적에 맞다. 8랙이 전부
    # 동일하므로 랙 번호는 결과에 영향하지 않는다 — 새 숫자를 만드는 것이 아니다.
    # **물리 가정이 아니라 시나리오 정의**다. 전 랙 동시 「막힘」은 5장에 근거가 없다.
    # 「샘」 쪽 모듈이 이 번호를 빌려 쓰지만 5-1 은 「「샘」 주입 지점」을 따로 두며
    # 성립 이유가 다르다(세션 7.27 · 대응표 `docs/leak-naming-map.md`).
    injection_rack_index: int = 0

    @property
    def k_increase_percent_levels(self) -> tuple[float, ...]:
        """5장 누출 3수준 [%] — 미량·중간·대규모."""
        return (
            self.k_increase_percent_minor,
            self.k_increase_percent_moderate,
            self.k_increase_percent_major,
        )

    @property
    def k_multiplier_levels(self) -> tuple[tuple[str, float], ...]:
        """**정상 + 누출 3수준**의 (이름, K 배율).

        정상이 배율 1.0 으로 이 목록에 함께 들어 있는 것이 요점이다 — 정상과 누출이
        **같은 코드 경로**로 돌아야 결과 차이가 누출 때문인지 경로 때문인지 갈라낼
        수 있다(세션 4 C2). 배율 1.0 은 "누출 없음"의 정의이지 새 가정치가 아니다.

        절대 규칙 8: 이 목록은 **「막힘」(배관저항 K값 증가)뿐**이다. 「샘」(질량
        손실)은 여기 없다 — 별도 모듈 `massloss.py`·`massloss_thermal.py` 가 풀고
        데이터셋 생성 경로에 들어가지 않는다. 수준 이름 문자열은 데이터셋에
        나간 것이라 바꾸지 않는다(세션 7.32).
        """
        return (("정상", 1.0),) + tuple(
            (f"누출 +{percent:g}%", 1.0 + percent / 100.0)
            for percent in self.k_increase_percent_levels
        )
