...
SCHEMA_DRAFT: tuple[tuple[str, str, str], ...] = (
    ...
    (
        ...
        "%",
        "6장 1-B 게이트 잔차. **「샘」 행은 빈 값**이다 — 누출 엔탈피 항 없이는 "
        "닫히지 않으므로(5-1 「「샘」 크기 수준」 한계 ⑷ · 세션 5.7-D) 이 행을 "
        "6장 기준으로 판정하지 않는다 [세션 7.39]",
    ),
    # ── solver 플래그 (절대 규칙 5) ──
...
MASSLOSS_PROVENANCE: dict[str, str] = {
    "signal_sign_caveat": (
        ...
        "**`energy_balance_residual_percent` 는 이 행에서 비어 있다** — 6장 1-B "
        "기준은 밀폐루프 잔차이고 「샘」은 누출 엔탈피 항 없이는 닫히지 않으므로"
        "(5-1 「「샘」 크기 수준」 한계 ⑷ · 세션 5.7-D: 빼면 최대 1.2129% · 넣으면 "
        "0.000062%) 이 행을 그 기준으로 판정하지 않는다. 빈 값을 「잔차 0」 또는 "
        "「검사 통과」로 읽지 않는다"
    ),
    "transient_caveat": (
        "**이 데이터셋에는 전이 행이 없다** — `regime` 이 전 행 `steady` 이고 "
