...
def count_combinations(
    ...
    return CombinationCount(
        ...
        + (" · 막힘 랙 축 켬" if leak_rack_axis else ""),
        steady_cases=base,
...
SCHEMA_DRAFT: tuple[tuple[str, str, str], ...] = (
    ...
    (
        ...
        "%",
        "0 / 5 / 20 / 50 — 5장 「막힘」 K값 증가율. **「샘」 행은 빈 값**이다 "
        "(K 배수를 쓰지 않는다 · 세션 7.39)",
    ),
    (
        "anomaly_rack_index",
        "int",
        "「막힘」 주입 랙. 8랙 대칭이라 결과 불변(5-1). **「샘」 행은 빈 값**이다 — "
        "「샘」은 랙에 국소화되지 않는다(5-1 「「샘」 주입 지점」 · 세션 7.39)",
    ),
    (
        "anomaly_cdu_index",
        "int",
    ...
    (
        ...
        "-",
        "「샘」 크기 스윕 지점 0.25/0.5/0.75/1.0 — 상한 대비 비율. "
        "「막힘」 행은 빈 값 [세션 7.39]",
    ),
    (
        "residual_return_share",
        "-",
        "구조 자유도 g — 잔여저항 중 누출점 하류(환수측) 몫 0/0.5/1.0. **전수로 "
        "돈다**(5-1 이 분해를 금지한다 · 미해결 #24). 「막힘」 행은 빈 값",
    ),
    (
        "pump_sees_supply_flow",
        "bool",
        "구조 자유도 — 펌프가 공급유량을 보는가 환수유량을 보는가. 전수. "
        "「막힘」 행은 빈 값(공급=환수라 이 자유도가 없다)",
    ),
    (
        "share_uses_return_flow",
        "bool",
        "구조 자유도 — 공유 2차측 배분을 환수유량으로 읽는가. 다중 CDU 「샘」 "
        "행에만 있다(#39)",
    ),
    ...
    (
        "total_flow_Lps",
        "L/s",
        "헤더 공급유량(랙이 받는 총유량). **부호 불확실** — 사람이 정한 것 1 참조",
    ),
    (
        "return_flow_Lps",
        "L/s",
        "헤더 환수유량. 「막힘」·정상은 밀폐루프라 `total_flow_Lps` 와 같고 "
        "**「샘」에서만 갈린다**(공급−환수 불일치 · 세션 5.6-B) [세션 7.39]",
    ),
    (
        "massloss_flow_Lps",
        "L/s",
        "「샘」 크기 — 5-1 「「샘」(질량손실) 크기 수준」의 역산으로 케이스마다 "
        "다시 나온다. 「막힘」 행은 빈 값(질량이 보존된다) [세션 7.39]",
    ),
    ...
    (
        ...
        "%",
        "6장 1-B 게이트 잔차. **「샘」 행도 채운다** [세션 7.53] — 기준·임계는 "
        "「막힘」·정상 행과 같고(`model.enthalpy_balance_residual_percent`), "
        "밀폐루프에서 하나였던 ṁ 이 「샘」에서 환수유량과 유출유량으로 갈리므로 "
        "뒤의 몫을 `massloss_enthalpy_kW` 로 넣는다(5-1 「「샘」 크기 수준」 "
        "한계 ⑷ · 세션 5.7-D)",
    ),
    # ── solver 플래그 (절대 규칙 5) ──
...
PROVENANCE_COLUMNS: tuple[tuple[str, str], ...] = (
    ...
    (
        "signal_sign_caveat",
        "이 행의 이상 기구는 「막힘」(배관 K값 증가 · 5장)이거나 없다(정상). "
        "「막힘」과 「샘」(계통 밖 질량손실)은 서로의 대용이 아니라 독립된 이상 "
        "상태이고, 「누출」은 둘을 묶는 상위 개념이다. 두 기구의 부호 관계는 "
        "기구 × 「샘」 주입점의 성질이며, 이 데이터셋의 「샘」 주입점은 랙 출구 "
        "하나다(2026-09-07 사람의 결정). 그 자리 · 정상상태 · 이상 기구를 진 "
        "CDU 에서 **「샘」은 총유량 · 주입랙 통과유량 · 주입랙 출구온도를 「막힘」과 "
        "반대 방향으로 움직인다**(「샘」의 구조 자유도 배치가 "
        "`residual_return_share`=0.0 · `pump_sees_supply_flow`=True 인 행의 수력 "
        "신호는 판정하지 않는다) — 이 행의 부호 규칙을 「샘」 행에 적용하지 않는다. "
        "이상 기구의 크기는 기구마다 다른 열에 담긴다 — 「막힘」은 "
        "`blockage_level_percent`(K값 증가율 %) · 「샘」은 "
        "`massloss_size_fraction`. 각 열은 다른 기구의 행에서 빈칸이다",
    ),
    (
        "transient_caveat",
        "**이 데이터셋에는 전이 행이 없다** — `regime` 이 전 행 `steady` 이고 "
        "`t_s` 가 전 행 빈칸이다(「샘」 전이는 미해결 #40 으로 열려 있다). "
        "아래는 전이 행이 실릴 때를 위한 유보이며 이 판본의 행에는 걸리지 않는다. "
    ...
    (
        "total_flow_caveat",
        "총유량만 보면 +50% 「막힘」(K값 +50%)도 −0.16~0.37% 라 놓칠 수 있다"
        "(세션 4 관측 ③). 랙 단위 열과 함께 읽는다",
    ),
    (
        "cr_caveat",
...
#: 「샘」(질량손실) 행에서 갈리는 표기 [세션 7.35]. 위 다섯 중 **셋만** 갈린다 —
#: `assumption_tag` 과 `cr_caveat` 은 기구와 무관해 그대로 쓴다(세션 7.34 C1 전수).
#:
#: 갈라야 하는 이유: 위 문언 셋은 **「막힘」(K값 증가 근사) 행을 전제로 쓰였고**,
#: 「샘」 행에 그대로 실으면 사실 오류(질량손실 해를 K 근사 결과라 부름)를 넘어
#: **부호를 거꾸로 읽게 지시한다**(절대 규칙 8 — 「샘」 주입점이 랙 출구일 때 ·
#: 미해결 #36 은 세션 7.68 에 닫혔다). 「샘」 행은 세션 7.39 부터 데이터셋에
#: 실리고, 그 행에는 아래 셋이 실린다.
MASSLOSS_PROVENANCE: dict[str, str] = {
    "signal_sign_caveat": (
        "이 행은 「샘」(계통 밖 질량손실) 해다 — 「막힘」(배관 K값 증가)이 아니다. "
        "두 기구는 서로의 대용이 아니라 독립된 이상 상태이고, 「누출」은 둘을 "
        "묶는 상위 개념이다. 두 기구의 부호 관계는 기구 × 「샘」 주입점의 "
        "성질이며, 이 데이터셋의 「샘」 주입점은 랙 출구 하나다(2026-09-07 사람의 "
        "결정). 그 자리 · 정상상태 · 이상 기구를 진 CDU 에서 총유량·이상 주입 랙 "
        "통과유량은 「막힘」이 줄이고 「샘」이 늘린다 — 「막힘」 행의 부호 규칙을 "
        "이 행에 적용하면 안 된다. 랙 유량·랙 출구온도도 안전하지 않다 — "
        "「샘」은 랙에 국소화되지 "
        "않아 랙 간 비대칭이 2.220e-16 L/s 에 그치고(5-1 「「샘」 크기 수준」), "
        "코드가 이상을 건 랙의 출구온도는 「막힘」과 부호가 갈린다. "
        "**`anomaly_rack_index` 는 이 행에서 빈칸이고, 그것이 사실에 맞다** — "
        "「막힘」 행에서 이 열은 저항이 걸린 랙을 가리키지만 「샘」은 랙에 "
        "국소화되지 않아(랙 간 비대칭 2.220e-16 L/s · 세션 5.6) 코드가 건 자리가 "
        "결과에 나타나지 않는다. 빈칸을 「랙을 못 골랐다」가 아니라 "
        "「가리킬 랙이 없다」로 읽는다. "
        "이상 기구의 크기는 기구마다 다른 열에 담긴다 — 「샘」은 "
        "`massloss_size_fraction` · 「막힘」은 `blockage_level_percent`"
        "(K값 증가율 %). 각 열은 다른 기구의 행에서 빈칸이다. "
        "구조 자유도가 g=0 · 펌프=공급 배치인 행에서 수력 다섯 양이 "
        "「정확히 0」이라는 것은 **수력 한정 · 물성 온도 37 ℃ 고정 기준선**에서의 "
        "말이다(세션 7.50 D4). 열까지 함께 풀린 이 데이터셋 행에서는 정확히 0 이 "
        "아니라 총유량 차가 1e-04 자리로 남으며, 갈리는 원인은 물성 평가 온도 "
        "하나다(세션 7.46). 어느 쪽이든 이 배치의 수력 신호는 판정 대상이 아니고"
        "(「해당 없음」 · 세션 7.47), 0 도 1e-04 자리도 「이상 없음」으로 읽지 "
        "않는다 — 온도는 이 배치에서도 응답한다. "
        "**`energy_balance_residual_percent` 는 이 행에서도 채워져 있다** "
        "[세션 7.53]. 세션 7.39~7.52 판본에서는 비어 있었다 — 두 판본을 나란히 "
        "읽지 않는다. 기준·임계는 「막힘」·정상 행과 **같은 것**이고"
        "(6장 ① <0.1% · `model.enthalpy_balance_residual_percent`), 밀폐루프에서 "
        "하나였던 ṁ 이 「샘」에서 환수유량과 계통 밖으로 나간 유량으로 갈리므로 "
        "뒤의 몫을 「샘」 엔탈피 항으로 넣어 더한다(5-1 「「샘」 크기 수준」 한계 ⑷ · "
        "세션 5.7-D). 물리 모델은 그대로다 — 항은 이미 해에 실려 있었다. "
        "이 잔차가 재는 것은 **상수 cp 선형화가 CoolProp 엔탈피 차와 어긋나는 "
        "크기** 하나이며, 「샘」 해가 물리적으로 옳다는 뜻이 아니다"
    ),
    "transient_caveat": (
        "**이 데이터셋에는 전이 행이 없다** — `regime` 이 전 행 `steady` 이고 "
        "`t_s` 가 전 행 빈칸이다(「샘」 전이는 미해결 #40 으로 열려 있다). "
        ...
        "이동했다. 그래도 절대 시간 규모는 해석하지 않는다 — M 결손(#21) · "
        "등가길이의 과대 방향 오차(#25) · 배관 규격 계열(#23)이 열려 있다. "
        "**질량손실 전이에는 유보가 하나 더 붙는다** — 계통이 실제로 줄어드는데 "
        "M 을 상수로 두므로, 상수 M 근사가 관측 창 뒷부분을 지탱하지 못한다"
        "(미해결 #38 · 세션 5.7-D). 창 뒤로 갈수록 파형을 덜 믿는다"
    ),
    "total_flow_caveat": (
        "「+50% 에서 −0.16~0.37%」는 「막힘」 실측이라 이 행에 대응하지 않는다"
        "(세션 4 관측 ③). 「샘」의 총유량은 반대 방향(증가)으로 움직이며 크기 "
        "수준은 5-1 「「샘」 크기 수준」의 역산 규칙을 따른다. **랙 단위 열과 "
        "함께 읽는 처방도 이 행에는 대응물이 없다** — 「샘」은 랙에 국소화되지 "
        "않아 랙 열이 어느 랙인지 가리키지 못한다. 총유량·환수유량·보유량 쪽을 "
        "함께 읽는다"
    ),
}
...
def format_plan_report() -> str:
    ...
    lines += [
        "",
        "(CDU 구성 3종 · 막힘 랙 축 끔 · 정상상태에는 M 축을 두지 않는다)",
        "",
        "── 실행시간 실측 ──────────────────────────────────────────────────",
    ]
    for sample in (
