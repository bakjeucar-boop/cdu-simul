...
PROVENANCE_COLUMNS: tuple[tuple[str, str], ...] = (
    ...
    (
        ...
        "총유량·양정은 그 근사의 결과이며, **다른 이상 기구인 「샘」(계통 밖 "
        "질량손실)은 같은 두 양을 반대 방향으로 움직인다**(미해결 #36) — 이 행의 "
        "부호 규칙을 「샘」 행에 적용하지 않는다. 랙 유량·랙 출구온도는 이 "
        "불확실성에 해당하지 않는다. "
...
#: **부호를 거꾸로 읽게 지시한다**(절대 규칙 8 · 미해결 #36). 「샘」 행이 아직
#: 데이터셋에 없어도 지금 갈라 둔다 — 행이 먼저 들어오면 그 순간부터 틀린
#: 표기가 실린다.
MASSLOSS_PROVENANCE: dict[str, str] = {
    "signal_sign_caveat": (
        "이 행은 질량손실(계통 밖 유출) 해다 — K값 증가 근사가 아니다. "
        "총유량·이상 주입 랙 통과유량의 부호가 「막힘」과 정반대이며"
        "(막힘은 둘 다 감소 · "
        "샘은 둘 다 증가 — 미해결 #36), 「막힘」 행의 부호 규칙을 이 행에 적용하면 "
        "안 된다. 랙 유량·랙 출구온도도 안전하지 않다 — 「샘」은 랙에 국소화되지 "
        ...
        "뒤의 몫을 누출 엔탈피 항으로 넣어 더한다(5-1 「「샘」 크기 수준」 한계 ⑷ · "
